defmodule AoC.Day09Explosives do
  ...
  def decompressed_length(:v2, compressed) do
    recursive_decompress_count(String.graphemes(compressed), 0)
  end
  ...
  defp recursive_decompress_count([], count), do: count

  defp recursive_decompress_count(["(" | rest], count) do
    {chars, rest} = read_number(rest)
    ["x" | rest] = rest
    {times, rest} = read_number(rest)
    [")" | rest] = rest
    ...
    str = rest |> Enum.take(chars) |> Enum.join("")
    rest = rest |> Enum.drop(chars)
    expanded = repeat(str, times) |> String.graphemes()

    recursive_decompress_count(expanded ++ rest, count)
  end

  defp recursive_decompress_count([_char | rest], count) do
    recursive_decompress_count(rest, count + 1)
  end
